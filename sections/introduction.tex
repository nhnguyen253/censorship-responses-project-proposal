...
The problem this project addresses is that we lack systematic visibility into this
form of censorship. While the network layer is measured continuously by OONI and
others, the app-store layer is measured far less consistently, even though it has
become one of the most powerful chokepoints for controlling access to VPNs, secure
messaging, and independent news. We built a measurement pipeline that queries the
per-country storefronts of the Apple App Store for a fixed basket of
censorship-sensitive apps, records which apps are available in which countries, and
cross-validates the results against transparency reports and documented events. In
this paper we report a single, carefully validated snapshot of 32 apps across 14
national storefronts. That snapshot independently reproduces every documented
removal we tested and yields a clean cross-country censorship signal
...
disproportionately high censorship~\cite{ververis2019shedding}. Our work extends
this line of work (Section~\ref{sec:related}): existing coverage is either
event-driven news reporting or focused narrowly on VPNs in Russia and China,
whereas we run a controlled, multi-category measurement with neutral controls and
pair it with an explicit treatment of the attribution problem, the question of
whether a missing app reflects government censorship rather than a sanctions-driven
withdrawal or a benign regional variant.
